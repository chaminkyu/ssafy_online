...
from django.contrib.auth.forms import (
    AuthenticationForm, # 로그인을 위한 폼 built-in form
    UserCreationForm, # 회원가입(db 저장, 변경) - model form
    PasswordChangeForm, # 비밀번호 변경 built-in form
)
...
def change_password(request, user_pk):
    ...
    if request.method == 'POST':
        ...
        # 부모 클래스인 SetPasswordForm의 생성자 함수 첫번째 인자가 user
        form = PasswordChangeForm(request.user, request.POST)
        if form.is_valid():
            user = form.save()
            # 비밀번호 변경하고 로그인 유지할지 안 할지
            update_session_auth_hash(request, user)
            # 로그인 유지하고 리다이렉트
            return redirect('articles:index')
        
    else:
        form = PasswordChangeForm(request.user) # 빈 폼 X(현재 로그인한 사용자)
