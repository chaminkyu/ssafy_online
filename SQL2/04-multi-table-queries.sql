...
CREATE TABLE articles (
    ...
    FOREIGN KEY (userId)  -- 외래키 설정
      REFERENCES users(id) -- users 테이블의 id(pk)를 참조조
);
...
-- JOIN 결과 후에 원본 테이블은 변경되지 않는다.
-- 만약 새로운 테이블로 저장하고 싶으면 CREATE TABLE
...
LEFT JOIN users
  ON users.id = articles.userId;
-- 2단계: 게시글이 없는 사용자 찾기(LEFT JOIN: 없는 데이터는 NULL값으로 채워짐)
SELECT * FROM users
LEFT JOIN articles
