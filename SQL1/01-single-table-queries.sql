...
-- 7. 다중 필드 정렬 문제
-- customer 테이블에서  Country를 내림차순으로,
--  정렬하여 조회하는 쿼리를 작성하세요.
SELECT
  Country, City
FROM
...
-- 13. AND 연산
-- customer 테이블에서 Company가 NULL이고, Country가 'USA'인 고객의
...
-- 14. OR 연산
-- customer 테이블에서 Company가 NULL이거나, Country가 'USA'인 고객의
-- LastName, FirstName, Company, Country를 조회하는 쿼리를 작성하세요.
SELECT
  LastName, FirstName, Company, Country
FROM
...
-- 19. LIKE 패턴 매칭 문제
-- customer 테이블에서 LastName이 'son'으로 끝나는 고객의
...
-- 20. LIKE 패턴 매칭 문제
-- customer 테이블에서 FirstName이 4글자이며 마지막 글자가 'a'인 고객의
-- LastName, FirstName을 조회하는 쿼리를 작성하세요.
SELECT
  LastName, FirstName
FROM
...
-- 과일가게가 있는데 손님이 "평균 가격이 5000원 미만인 과일을 담아주세요"
-- 1단계 : 과일을 종류별로 그룹지어야 한다(사과, 배, 파인애플...)
-- 2단계 : 각 그룹(과일 종류)의 평균 가격을 계산
-- 3단계 : 마지막으로 평균 가격이 5000원 미만인 그룹을 선택해서 담는다.

-- WHERE을 쓴다? --> 그룹짓기도 전에(1단계 전에) 평균 가격을 모르는 상태에서 필터링
-- 결론 : 에러가 난다
-- HAVING --> 2단계 이후에 평균 가격을 알고 있는 상태에서 필터링

-- 25. WHERE <-> HAVING 차이
-- tracks 테이블에서 Composer별로 그룹화하여
-- 각 작곡가의 평균 재생시간(Milliseconds/60000)을 계산하고,
-- 평균 재생 시간이 10분 미만인 작곡가만 조회하는 쿼리를 작성하세요.
SELECT
  Composer,
